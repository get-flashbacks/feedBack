...
- **Hand practice filter**: settings → Hands persists `Both`, `Left`, or `Right` in `keys3d_hand_filter`. Explicitly hidden `lh`/`rh` notes are omitted from rendering and hit/miss scoring; playing one is neutral rather than a wrong-note miss. Unlabelled and nonstandard-staff notes stay visible and scoreable in every mode.
