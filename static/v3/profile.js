...
(function () {
    ...
    function show(profile, opts) {
        ...
        async function saveSongDirStep() {
            submit.disabled = true;
            try {
                await saveSongDir();
                setStep(3);
        ...
        async function saveAmpSimsStep() {
            submit.disabled = true;
            try {
        ...
        submit.addEventListener('click', async () => {
            ...
            if (step === 1) { advanceToStep2(); return; }
            if (step === 2) { await saveSongDirStep(); return; }
            if (step === 3) { await saveOptInStep(); return; }
            if (step === 4) { await savePathsStep(); return; }
            if (step === 5) { await saveAmpSimsStep(); return; }
            await launchPlay();
        });
