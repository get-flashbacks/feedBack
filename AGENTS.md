...
- `highway.hasPhraseData()` — returns `true` only when at least one phrase actually has more than one difficulty level (i.e. the mastery slider can change what's rendered somewhere in the song, and `getFilteredNotes()` / `getFilteredChords()` return a filtered subset). Use this to gate logic that only makes sense when difficulty filtering is available. **Not** the same as "phrase data exists" — see `getPhrases()` below.
- `highway.getPhrases()` — phrase timing windows `[{ index, start_time, end_time, max_difficulty, top_difficulty }]` for the current song. `max_difficulty` is the phrase's tier scale (the slider picks tier `floor(mastery * (max_difficulty + 1))` and the phrase plays its last level whose `difficulty` is ≤ that tier); `top_difficulty` is the tier from which the phrase plays in full — equal to `max_difficulty` for a fully authored ladder, lower when duplicate tiers were collapsed or a generated phrase is complete early. Returns `null` only when the source has no phrase data at all (GP imports, single-difficulty charts). A chart whose declared difficulty levels never actually differ (e.g. an unsimplified CDLC arrangement) still returns real timing windows here — each phrase's ladder just collapses to one level server-side — so `getPhrases()` can be non-null even when `hasPhraseData()` is `false`. Don't assume the two are equivalent; pair `hasPhraseData()` with logic that needs a real ladder, and use `getPhrases()` on its own for phrase-boundary-only needs (e.g. Section Practice's phrase-sized looping). Read-only; do not mutate.
...
- **Auto-sync** — `.github/workflows/sync-version.yml` rewrites `VERSION` via a `repository_dispatch` (`desktop-released`) fired from `feedBack-desktop`'s release job. As an explicit automation-only exception to the "Never push directly to main" rule in Git Workflow below, the sync job commits straight to `main` as `github-actions[bot]`. This is safe not because the change is "mechanical" — a PR round-trip provides more than review signal, it also runs required CI before merge, which the direct commit bypasses — but because the input is validated before it's ever written: both the `repository_dispatch` payload and the `workflow_dispatch` `version` input are anchored against `^[0-9]+\.[0-9]+\.[0-9]+$` (after stripping an optional leading `v`) and the job fails hard on anything else (e.g. a stray `soundfonts-v1` tag), and CI still runs post-merge on `main` regardless. Human contributors must still go through feature branches + PRs. No manual VERSION edits needed. Use the workflow's `workflow_dispatch` trigger with `version: X.Y.Z` for manual runs (recovery / out-of-band bumps) — a malformed value is rejected by the same semver guard rather than landing on `main`.
